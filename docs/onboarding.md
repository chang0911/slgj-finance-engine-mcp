...
> 版本：v1.3（2026-09-24 同步生产 26 工具）　适用对象：希望通过 AI 客户端（Claude / Cursor / VS Code / CodeBuddy / Cherry Studio 等）调用丝路E投财务引擎的用户

丝路e投财务模型引擎把全寿命财务模型能力开放为标准 MCP 服务：**26 个工具**覆盖结构化数据采集（intake 宽进）、财务测算、指标反算、借款还本付息计算、专项债融资收益平衡精算、Excel/看板/Word/PPT 成品生成、投资估算、准许成本定价、国民经济评价、不确定性分析、报表勾稽体检与收入费用审查。接入后，在 AI 客户端里直接用自然语言描述需求即可完成建模、测算与成品交付。
...
| get_skill_instructions | 读取技能工作流指令模板（快速测算/估算/演示/定价/可研/经济评价/专项债审查/后评价/数据溯源等 11 类） | — |
...
| run_bond_review 🔑 | 专项债融资收益平衡精算：覆盖倍数（毛/净/申报/资本金/组合融资）+分年平衡表；五模块审查流程见 get_skill_instructions(skill=bond_review) | 需 |
