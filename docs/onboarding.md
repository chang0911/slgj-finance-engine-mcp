...
丝路e投财务模型引擎把全寿命财务模型能力开放为标准 MCP 服务：**21 个工具**覆盖财务测算、指标反算、Excel/看板/Word/PPT 成品生成、投资估算、准许成本定价、国民经济评价、不确定性分析、报表勾稽体检与收入费用审查。接入后，在 AI 客户端里直接用自然语言描述需求即可完成建模、测算与成品交付。
